...
with st.sidebar:
    ...
    diagnostics = st.toggle("Diagnostics mode", value=False, help="Show stack traces and environment checks if errors occur.")
...
if uploaded_files:
    ...
    if not api_key:
        st.error("No OpenAI API key found. Set `st.secrets['OPENAI_API_KEY']` or the `OPENAI_API_KEY` environment variable.")
            ...
            for idx, ch in enumerate(chunks, start=1):
                try:
                    ...
                    if diagnostics:
                        ...
                        st.error(f"LLM analysis failed on chunk {idx}. Enable Diagnostics mode for details.")
...
".join(html_parts).encode("utf-8")
            ...
            if diagnostics:
                ...
                st.error(f"Something went wrong while processing {uf.name}. Enable Diagnostics mode for details.")
